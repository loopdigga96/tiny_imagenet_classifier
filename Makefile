VERSION=v1.6.1
BASE_ENV_VERSION=v1.7.4
PROJECT_ID=neuro-project-617ae022
...
INFERENCE_JOB?=inference-$(PROJECT)
...
# Inference command (to be changed):
INFERENCE_CMD?=flask run
# port inside the container to expose (to be changed):
INFERENCE_PORT?=8080

.PHONY: inference
inference: _check_setup $(SYNC) ### Run the inference job
	$(NEURO) run \
		--name $(INFERENCE_JOB) \
		--preset $(PRESET) \
		--http $(INFERENCE_PORT) \
...
.PHONY: kill-inference
kill-inference:  ### Terminate the inference job
	$(NEURO) kill $(INFERENCE_JOB) || :
